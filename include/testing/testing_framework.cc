...
void ControlStatsSnapshot::print() const {
  ...
  std::cout << "----------------------------" << std::endl;
    ...
    unsigned long long ControlStatsSnapshot::total_comparisons() const {
         return less_than_count + greater_than_count + less_equal_count + greater_equal_count + equal_count + not_equal_count;
    }

    // Helper to get total copy/move operations (often indicative of swaps/data movement)
     unsigned long long ControlStatsSnapshot::total_data_moves() const {
         return copy_constructor_count + copy_assignment_count + move_constructor_count + move_assignment_count;
     }
...
void Control::increment_less_than() {
  ++less_than_count;
}

void Control::increment_greater_than() {
  ++greater_than_count;
}

void Control::increment_less_equal() {
  ++less_equal_count;
}

void Control::increment_greater_equal() {
  ++greater_equal_count;
}

void Control::increment_equal() {
  ++equal_count;
}

void Control::increment_not_equal() {
  ++not_equal_count;
}

void Control::increment_copy_constructor() {
  ++copy_constructor_count;
}

void Control::increment_copy_assignment() {
  ++copy_assignment_count;
}

void Control::increment_move_constructor() {
  ++move_constructor_count;
}

void Control::increment_move_assignment() {
...
void Control::print_stats() const {
  ...
  std::cout << "  Copy Constructions: " << copy_constructor_count
            << std::endl;
  std::cout << "  Copy Assignments  : " << copy_assignment_count << std::endl;
  std::cout << "  Move Constructions: " << move_constructor_count
            << std::endl;
...
Testing &Testing::operator=(const Testing &other) {
  ...
  value = other.value;
  // Assuming control pointer is already initialized correctly in the existing object
  if (control) { // Good practice to check if control is not null
      ...
      // Or initialize it: control = &Control::get_instance(); control->increment...
  }
  return *this;
}
...
Testing::Testing(Testing &&other) noexcept
    : value(other.value), control(&Control::get_instance()) { // Note: Member initialization list is generally preferred
